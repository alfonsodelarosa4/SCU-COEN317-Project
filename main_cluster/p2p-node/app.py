...
if __name__ == "__main__":
    ...
    app.logger.debug("USERS")
    user_id = create_user(name="Tom Hanks", email="tomhanks@gmail.com")
    users = get_users()
    app.logger.debug("-retrieved users")
    app.logger.debug(users)

    update_user(user_id,name="Keanu Reeves",email="keanureeves@gmail.com")
    app.logger.debug("-updated user")
    user = get_user(user_id)
    app.logger.debug(user)

    delete_user(user_id)
    app.logger.debug("deleted user")
    users = get_users()
    app.logger.debug("retrieved users")
    app.logger.debug(users)

    app.logger.debug("LEADER")
    app.logger.debug("-no leader")
    leader = get_leader()
    app.logger.debug(leader)

    set_leader("1234")
    app.logger.debug("-new leader")
    leader = get_leader()
    app.logger.debug(leader)

    app.logger.debug("SUBSCRIBED TOPICS")
    app.logger.debug("-no subscribed topics")
    topics = get_subscribed_topics()
    app.logger.debug(topics)

    app.logger.debug("-adding food")
    create_subscribed_topic("food")
    topics = get_subscribed_topics()
    app.logger.debug(topics)

    app.logger.debug("-get food")
    topic = get_subscribed_topic("food")
    app.logger.debug(topic)

    app.logger.debug("-deleted subscribed topics")
    topic = delete_subscribed_topic("food")
    topics = get_subscribed_topics()
    app.logger.debug(topics)

    app.logger.debug("TOPIC NEIGHBORS")

    app.logger.debug("-add topic neighbor")
    create_topic_neighbor("789","food")
    create_topic_neighbor("123","food")
    create_topic_neighbor("789","tech")
    create_topic_neighbor("123","tech")
    create_topic_neighbor("567","tech")

    app.logger.debug("-get neighbors from a topic")
    neighbors = get_topic_neighbors("food")
    app.logger.debug(neighbors)
    app.logger.debug("-get all neighbors")
    neighbors = get_topic_neighbors_from_all_topics()
    app.logger.debug(neighbors)

    delete_neighbor_from_all_topics("123")
    app.logger.debug("-123 was deleted from food")
    neighbors = get_topic_neighbors("food")
    app.logger.debug(neighbors)
    app.logger.debug("-123 was deleted from tech")
    neighbors = get_topic_neighbors("tech")
    app.logger.debug(neighbors)

    delete_all_neighbors_from_a_topic("tech")
    app.logger.debug("- all neighbors in tech were deleted")
    neighbors = get_topic_neighbors("tech")
    app.logger.debug(neighbors)

    app.logger.debug("POSTS")
    app.logger.debug("create post")
    post_id = create_post(topic="food",ip_address="1234",text="hello world",timestamp="1234")
    app.logger.debug("get posts")
    posts = get_posts_by_topic("food")
    app.logger.debug(posts)
